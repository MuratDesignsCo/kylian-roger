...
function HomepageEditor() {
  ...
  const loadData = async () => {
    ...
    if (heroResult.error || featuredResult.error || hoverResult.error || projectsResult.error) {
      toast.error('Erreur lors du chargement des donnees')
      console.error(heroResult.error, featuredResult.error, hoverResult.error, projectsResult.error)
      setLoading(false)
      return
    }
    ...
    if (heroResult.data) {
      setHeroImages(
        heroResult.data.map((img: HeroImage) => ({
          tempId: genTempId(),
    ...
    if (featuredResult.data) {
      const featured = featuredResult.data as HomepageFeaturedWork[]
    ...
    if (hoverResult.data) {
      ...
      setHoverImages(
        identifiers.map((id) => {
          const existing = (hoverResult.data as AboutHoverImage[]).find(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // --- Hero Images: delete all, insert new ---
      const { error: deleteHeroErr } = await supabase
        .from('hero_images')
        .delete()
        .gte('id', '00000000-0000-0000-0000-000000000000')

      if (deleteHeroErr) throw new Error(deleteHeroErr.message)

      if (heroPayload.length > 0) {
        const { error: insertHeroErr } = await supabase
          .from('hero_images')
          .insert(heroPayload)
        if (insertHeroErr) throw new Error(insertHeroErr.message)
      }

      // --- Featured Works: delete all, insert new ---
      const { error: deleteFeatErr } = await supabase
        .from('homepage_featured_works')
        .delete()
        .gte('id', '00000000-0000-0000-0000-000000000000')

      if (deleteFeatErr) throw new Error(deleteFeatErr.message)

      if (allFeatured.length > 0) {
        const { error: insertFeatErr } = await supabase
          .from('homepage_featured_works')
          .insert(allFeatured)
        if (insertFeatErr) throw new Error(insertFeatErr.message)
      }

      // --- Hover Images: update each by link_identifier ---
      for (const img of hoverPayload) {
