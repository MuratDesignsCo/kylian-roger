...
import { ADMIN_PHOTOGRAPHY_PAGE_QUERY } from '@/lib/graphql/queries'
import {
...
function PhotographyEditor() {
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [activeTab, setActiveTab] = useState<'content' | 'seo'>('content')

  // Page settings
  const [pageTitle, setPageTitle] = useState('PHOTOGRAPHY')
  ...
  const loadData = async () => {
    ...
    try {
      ...
      }>(ADMIN_PHOTOGRAPHY_PAGE_QUERY)

      if (data.pageSettings) {
        setPageTitle(data.pageSettings.page_title || 'PHOTOGRAPHY')
        setItemsPerPage(data.pageSettings.items_per_page ?? 9)
      }
        ...
        <div className="admin-section">
          ...
          <div>
            ...
            <input
              ...
              max={50}
              value={itemsPerPage}
              onChange={(e) => setItemsPerPage(Math.max(1, Number(e.target.value) || 9))}
              className="w-32 rounded-lg border border-gray-200 px-4 py-2.5 text-sm focus:border-black focus:outline-none"
            />
            <p className="mt-1.5 text-xs text-gray-400">
              Nombre de projets affichés avant le bouton &quot;Load More&quot; (par défaut : 9)
            </p>
          </div>
        </div>
      )}
