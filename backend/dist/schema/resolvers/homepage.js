...
export const homepageResolvers = {
    ...
    Mutation: {
        updateHomepage: async (_, { input }, context) => {
            requireAuth(context);
            // Hero images: delete all, re-insert
            if (input.heroImages) {
                await pool.query('DELETE FROM hero_images');
                const images = input.heroImages;
                for (const img of images) {
                    await pool.query('INSERT INTO hero_images (image_url, alt_text, sort_order) VALUES ($1, $2, $3)', [img.image_url, img.alt_text || '', img.sort_order || 0]);
                }
            }
            // Featured works: delete all, re-insert
            if (input.featuredWorks) {
                await pool.query('DELETE FROM homepage_featured_works');
                const works = input.featuredWorks;
                for (const fw of works) {
                    await pool.query('INSERT INTO homepage_featured_works (project_id, slot_category, slot_index, sort_order) VALUES ($1, $2, $3, $4)', [fw.project_id, fw.slot_category, fw.slot_index, fw.sort_order || 0]);
                }
            }
            // Hover images: update each by link_identifier
            if (input.hoverImages) {
                const images = input.hoverImages;
                for (const img of images) {
                    await pool.query('UPDATE about_hover_images SET image_url = $1, alt_text = $2 WHERE link_identifier = $3', [img.image_url, img.alt_text || '', img.link_identifier]);
                }
            }
