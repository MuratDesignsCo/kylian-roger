...
export const seoResolvers = {
    Query: {
        ...
        seoPage: async (_, { slug }) => {
            const { rows } = await pool.query('SELECT * FROM pages_seo WHERE page_slug = $1', [slug]);
            return rows[0] || null;
        },
    },
    Mutation: {
        upsertSeoPages: async (_, { input }, context) => {
            requireAuth(context);
            const results = [];
            for (const page of input) {
                if (page.id) {
                    const { rows } = await pool.query(`UPDATE pages_seo SET
            ...
            WHERE id = $7 RETURNING *`, [
                    ...
                    const { rows } = await pool.query(`INSERT INTO pages_seo (page_slug, meta_title, meta_description, og_title, og_description, og_image_url)
            VALUES ($1, $2, $3, $4, $5, $6)
